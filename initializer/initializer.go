...
var DefaultConfiguration = model.ExecutorConfig{
	...
	ContainerServiceProvider:           "garden",
}

func Initialize(logger lager.Logger, config model.ExecutorConfig, gardenHealthcheckRootFS string, metronClient loggingclient.IngressClient, clock clock.Clock) (executor.Client, grouper.Members, error) {
	...
	gardenClient := vgarden.NewWithAdapter(gardenClientInner, logger, config.ContainerProviderConfig)
